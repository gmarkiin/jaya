...
keys: ## Generate secret keys
	docker compose exec jaya_nginx bash -c "su -c 'php artisan key:generate' application"
...
unlog: ## Clear log file
	docker compose exec jaya_nginx bash -c "echo '' > storage/logs/laravel.log"

tinker: ## Start tinker
	docker compose exec --user application jaya_nginx bash -c "php artisan tinker"

##@ Bash shortcuts

bash: ## Enter bash jaya_nginx container
...
autoload: ## Run the composer dump
	docker compose exec --user application jaya_nginx bash -c "composer dump-autoload"
